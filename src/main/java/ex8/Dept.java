 ...
 * 다음은 자바빈의 규약입니다.
 - 1. 필드 마다 별도의 get/set 메서드가 존재헤야 한다.
   2. get 메서드는 파라미터가 존재하기 않아야 한다.
   3. set 메소드는 반드시 하나 이상의 파라미터가 존재해야 한다.
   4. 빈즈 컴포넌트의 속성은 반드시 일기 또는 쓰기가 가능해야 한다.
      단, 일기 전용인 경우 get 또는 is 메서드만 정의할 수 있다.
   ...
   * 자바빈 클래스 데이터를 저장하는 필드, 데이터를 읽어올 때 사용되는
     메서드(getter메서드), 값을 저장할 때 사용되는 메서드(setter메서드)로 구성됩니다.
     
   * 메서드 이름을 사용해서 프로퍼티(property)의 이름을 결정하게 됩니다.
     예를 들어 프로퍼티딍 이름이 name이고 값의 타입이 int인 경우 프로퍼티와 관련된 메서드의 이름은 다음과 같이 결정됩니다.
     public void setName(int value){};
     public int getName(){};
     
     setName()과 getName()을 통해서 관리되는 데이터를 프로퍼티(property)라고 부릅니다.
 ...
 ***	데이터 빈(DataBean) 클래스 작성 	***
   ...
   DB에서 만들었던 칼럼명과 동일하게 프로퍼티들을 생성합니다.
 */
public class Dept {
	private int deptno;
