...
<%
	...
	//sFilePath에 있는 파일의 MinType을 구해옵니다.
	...
	/*
		...
		2. Content-Type을 통해서 브라우저는 해당 테이터를 어떻게 처리해야 할 지 판달할 수
			있게 됩니다.
			(예)
				1) image/png: 브라우저는 해당 컨텐트를 이미지로써 간주하게 됩니다.
				2) application/octet-stream: 미확인 Binary 정보를 의미하며,
								이 경우 브라우저는 파일을 다운로드하는 형태로 동작합니다.
				3) text/javascript:브라우는 Content를 javascript문서로 취급하게 됩니다.				
	*/
	
	/*
		octet-stream은 8비트로 된 데이터를 의미하여 지정되지 않은 파일 형식을 의미합니다.
	*/
	if (sMimeType == null)
	...
	/*
		- Content-Disposition: Content가 어떻게 처리되어야 하는지 나타냅니다.
		  1) Content-Disposition:
			 inline: 브라우저가 Content가 어떻게 처리되어야 하는지 나타냅니다.
	...
	*/
	...
	2. try-with-resource문에 의해 자동으로 객체의 clise()가 호출될 수 있으려면 클래스가
	   AutoCloseable이라는 인터페이스를 구현한 거이어야 합니다.
	형식) try(){
		 .....
		}catch(){
	...
	*/
	...
	try (
			...
			)
			{
				...
				//read(b, 0, b.leangth) : 바이트 배열 b의 0번 부터 b.leangth 크기 만큼 읽어옵니다.
